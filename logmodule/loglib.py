...
from logmodule.clienthandler import ClientHandler
...
class LogLib:
    ...
    @staticmethod
    def addClientHandle(clientmgr):
        if LogLib.chdlr != None:
            return False
        
        LogLib.chdlr = ClientHandler(clientmgr)
        formatter = logging.Formatter('%(asctime)s %(levelname)s %(module)s %(lineno)d %(message)s')
        LogLib.chdlr.setFormatter(formatter)
        LogLib.logger.addHandler(LogLib.chdlr)
        
        return True
    '''
    @staticmethod
    def addConcurrentRotatingFileHandler(filename):
        #使用下面的handle需要另外安装pip install ConcurrentLogHandler
        #多进程环境下使用
        #from cloghandler import ConcurrentRotatingFileHandler
        try:
            if __name__ == '__main__':
                #for test
                LogLib.fhdlr = ConcurrentRotatingFileHandler(filename, 'a', 128*1024, 5)
            else:
                LogLib.fhdlr = ConcurrentRotatingFileHandler(filename, 'a', 1024*1024, 100)
            ...
            print('LogLib addConcurrentRotatingFileHandler exception:%s' % (str(data)))
            return False
        '''
    @staticmethod
    def writeToFile():
