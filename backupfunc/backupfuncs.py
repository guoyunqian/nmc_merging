...
def get_func_params(funcname, cfg, section, params):
    if funcname == backupfunc_forword.func_name:
        backupfunc_forword.get_params(cfg, section, params)
    elif funcname == backupfunc_datasource.func_name:
        backupfunc_datasource.get_params(cfg, section, params)
    elif funcname == backupfunc_missing.func_name:
        backupfunc_missing.get_params(cfg, section, params)
    else:
...
def run_func(funcname, params):
    if funcname == backupfunc_forword.func_name:
        backupfunc_forword.run_func(params)
    elif funcname == backupfunc_datasource.func_name:
        backupfunc_datasource.run_func(params)
    elif funcname == backupfunc_missing.func_name:
        backupfunc_missing.run_func(params)
    else:
        raise Exception('unknown function name')
    '''
