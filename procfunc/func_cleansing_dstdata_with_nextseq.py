#!/usr/bin/python
# -*- coding: utf-8 -*-
#Filename: func_cleansing_dstdata_with_nextseq.py
...
func_name = 'cleansing_dstdata_with_nextseq'

#读cleansing_dstdata_with_nextseq函数需要的参数
def get_params(cfg, section, params, logger):
    #处理过程需要用到的数据集合
    #rst = public.get_opt_str(cfg, section, 'data')
    #if rst is None or len(rst) == 0:
    #    raise Exception('cleansing_dstdata_with_nextseq %s data error' % section)

    params[FixParamTypes.DatasName] = None

    #往后找seq的次数
    rst = public.get_opt_int(cfg, section, 'interval')
    if rst is None:
        raise Exception('cleansing_dstdata_with_nextseq %s interval error' % section)
        
    params[FixParamTypes.TNum] = rst

    #对应时效没有数据时如何处理
    rst = public.get_opt_float(cfg, section, 'miss_value')
    if rst is None:
        raise Exception('cleansing_dstdata_with_nextseq %s miss_value error' % section)
    ...
    if rst is None:
        raise Exception('cleansing_dstdata_with_nextseq %s seq error' % section)
...
def set_func_params(save_dt, func_params, src_datas, savecfginfos, dst_datas, logger):
    params = {}
    params[FixParamTypes.GridDataList] = None #src_datas[func_params[FixParamTypes.DatasName]]
    ...
    params[FixParamTypes.TNum] = func_params[FixParamTypes.TNum]
...
def run_func(params, logger):
    ...
    interval = params[FixParamTypes.TNum]
    ...
    for s in seq:
        ...
        for i in range(1, interval+1):
            c = s + i
            if c in dst_datas:
                dst_datas[s] = dst_datas[c]
                break

        if s not in dst_datas:
