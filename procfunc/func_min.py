...
def get_params(cfg, section, params, logger):
    ...
    if rst is None or len(rst) == 0:
        raise Exception('min %s data error' % section)
    ...
    if rst is None or len(rst) == 0:
        raise Exception('min %s compare_data error' % section)
    ...
    #if rst is None:
    #    raise Exception('min %s miss_value error' % section)
    ...
    for s in dst_datas.keys():
        for curseq in range(s - seqdelta, s+1):
            if curseq in datas.keys():
                dst_datas[s].values = np.fmin(datas[curseq].values, dst_datas[s].values)
